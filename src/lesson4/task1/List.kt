 ...
 */
var ed = listOf<String>("один", "два", "три", "четыре", "пять", "шесть", "семь", "восемь", "девять")
var des = listOf<String>(
...
var edt = listOf<String>(
    "тысяч", "одна тысяча", "две тысячи", "три тысячи", "четыре тысячи", "пять тысяч", "шесть тысяч",
    "семь тясяч", "восемь тысяч", "девять тысяч"
)
var dest = listOf<String>(
...
fun russian(n: Int): String {
    ...
    for (i in 0 until digitNumber(n)) {
        when (i) {
            0 -> if ((s[1] != 1) && (s[0] != 0)) r.add(ed[s[i] - 1])
            1 -> if (s[1] != 0)
                if (s[1] != 1) r.add(des0[s[1] - 2]) else r.add(des[s[0]])
            2 -> if (s[2] != 0) r.add(sot[s[i] - 1])
            3 -> if ((s[4] != 1)) r.add(edt[s[3]])
            4 -> if (s[4] != 0)
                if (s[4] != 1) r.add(des0[s[4] - 2]) else r.add(dest[s[3]])
            5 -> r.add(sot[s[i] - 1])
        }
    }
    r.reverse()
